...
const generator = async () => {
  ...
  const openapi = await readOpenapi(path.resolve(currentPath, openapiPath));
  return Promise.all(automatons.map(async automatonSetting => {
    ...
    return generator(openapi, {outDir: automatonSetting.outDir, path: currentPath, openapiPath}, automatonSetting.options);
  }));
};
